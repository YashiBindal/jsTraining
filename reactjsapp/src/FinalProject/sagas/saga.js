...
import { SecureHttpService } from "../../services/SecureService";
...
function createUser(user) {
  let service = new SecureHttpService();
  // const response = service.createUser(user).then((result) => result.data);
  const response = service.createUser(user).then((res) => {
...
function* createUserSuccess(action) {
  ...
  // const newAction =  createUser(newUser)
...
//9.
function* listenToCreateUserDispatchAction() {
  console.log("listening to create user action");
  yield takeLatest("ADD_USER", createUserSuccess);
}
// last step
...
export default function* rootSaga() {
  ...
  yield all([
    listenToCreateUserDispatchAction(),
